...
def generate_equivalents(packs, max_distance=50):
    ...
    for source_brand_id, colors in packs.items():
        for color in colors:
            ...
            other_candidates = [
                (bid, c, h) for bid, c, h in candidates 
                if bid != source_brand_id
            ]
...
def main():
    ...
    # Show mr_hobby stats if available
    if "mr_hobby" in equivalents:
        mr_hobby_equivs = len(equivalents["mr_hobby"])
        mr_hobby_with_equivs = sum(1 for codes in equivalents["mr_hobby"].values() if codes)
        print(f"\n   Mr. Hobby:")
        print(f"     • Colors with equivalents: {mr_hobby_with_equivs}")
        print(f"     • Total equivalences: {sum(len(e) for e in equivalents['mr_hobby'].values())}")
        
        # Show Federal Standard matches for Mr. Hobby
        if "federal_standard" in equivalents.get("mr_hobby", {}):
            print(f"     • Federal Standard matches: {len(equivalents['mr_hobby']['federal_standard'])}")
