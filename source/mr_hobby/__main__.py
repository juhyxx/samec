...
def extract_color_names(text_entries, cells, matches):
    ...
    for match in matches:
        ...
        if name_texts:
            ...
            name = "Unnamed"
        # If OCR picked very short uppercase abbreviations,
        # treat them as layout artifacts.
...
def normalize_color_name(name):
    """Replace short OCR artifacts with a stable placeholder name."""
...
def finalize_colors(colors):
    """Apply final pack-level cleanup before writing output."""
    return [
        {
            "code": color.get("code", ""),
...
if __name__ == "__main__":
    ...
    parsed_colors = parse_mr_hobby_images(source_folder, output)
